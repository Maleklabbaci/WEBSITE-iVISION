...
const StaticBackground: React.FC = () => (
  <div className="fixed top-0 left-0 w-full h-full z-[-1] bg-brand-dark">
    <div
      ...
      style={{
        backgroundImage:
          "radial-gradient(circle at 20% 30%, rgba(56, 189, 248, 0.08) 0%, transparent 50%), radial-gradient(circle at 80% 70%, rgba(56, 189, 248, 0.05) 0%, transparent 50%)",
...
const App: React.FC = () => {
  ...
  const handleSelectLanguage = (selectedLanguage: Language) => {
    ...
    setTimeout(() => setShowLangSelector(false), 600);
  };
    ...
    <div className="min-h-screen relative font-sans overflow-x-hidden">
      ...
      {!isLoading && showLangSelector && (
        <div
          ...
          className={`fixed inset-0 z-[100] flex items-center justify-center p-6 bg-brand-dark/40 backdrop-blur-xl transition-all duration-700 ${
            isExitingLangSelector
      ...
      <div
        key="main-content"
        className={`relative z-10 flex flex-col min-h-screen transition-all duration-1000 ${
          isLoading ? "opacity-0 scale-95" : "opacity-100 scale-100"
        } ${showLangSelector ? "filter blur-md" : "filter blur-0"}`}
      >
        ...
        <main className="flex-grow pt-24">
          ...
          {currentView === "quote" && <QuoteForm translations={t.contact} />}
