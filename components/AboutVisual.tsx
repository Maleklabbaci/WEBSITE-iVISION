...
const copy = {
  ...
  en: { eyebrow: 'ABOUT / 01', title: 'We turn attention into growth.', body: 'iVISION builds clear digital systems: a strong idea, a precise image and acquisition you can measure.', principles: ['A direction people notice.', 'Content that earns trust.', 'An experience that converts.'], link: 'See our work', trust: 'TEAMS THAT TRUST OUR WORK' },
  ar: { eyebrow: 'من نحن / ٠١', title: 'نحوّل الانتباه إلى نمو.', body: 'نبني أنظمة رقمية واضحة: فكرة قوية، صورة دقيقة واكتساب يمكن قياسه.', principles: ['اتجاه يلفت الانتباه.', 'محتوى يبني الثقة.', 'تجربة تحقق النتائج.'], link: 'اكتشف أعمالنا', trust: 'فرق تثق في عملنا' },
};
  ...
  return (
    <section ref={ref} id="about" className={`iv-about ${visible ? 'is-visible' : ''}`} dir={language === 'ar' ? 'rtl' : 'ltr'}>
      ...
      <div className="iv-about-grid">
        ...
        <div className="iv-about-side"><ol>{t.principles.map((principle, index) => <li key={principle}><span>0{index + 1}</span><p>{principle}</p></li>)}</ol></div>
      </div>
