  ...
  return (
    ...
    >
      <div className="flex justify-between items-center text-left">
        ...
        <div className={`w-12 h-12 rounded-xl flex items-center justify-center border border-white/10 dark:border-white/10 transition-all duration-500 ${isOpen ? 'bg-brand-blue border-brand-blue rotate-45' : 'bg-white/5 dark:bg-white/5'}`}>
           <svg className={`w-6 h-6 transition-colors ${isOpen ? 'text-white' : 'text-white'}`} fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={3} d="M12 6v6m0 0v6m0-6h6m-6 0H6" /></svg>
        </div>
      </div>
      <div
        ref={contentRef}
      ...
      >
        <div className="pt-8 text-brand-gray dark:text-brand-gray/80 text-lg font-medium leading-relaxed opacity-80 max-w-4xl border-t border-white/5 dark:border-white/5 mt-8">
          {qa.answer}
        </div>
      </div>
    </div>
...
const FAQ: React.FC<FAQProps> = ({ translations }) => {
  ...
  return (
    <section id="faq" ref={sectionRef} className="py-40 bg-white/0 dark:bg-transparent relative border-t border-white/5 dark:border-white/5 overflow-hidden transition-colors duration-500">
        ...
        <div className={`mb-32 transition-all duration-1000 ${isVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-10'}`}>
          <div className="flex flex-col md:flex-row justify-between items-end gap-12">
            <div className="max-w-4xl">
              ...
              <h2 className="text-5xl md:text-[8rem] font-black text-white tracking-tighter leading-[0.8] uppercase transition-colors duration-500">
