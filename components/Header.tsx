...
const Header: React.FC<HeaderProps> = ({ translations, onQuoteClick, theme, onToggleTheme, language, onChangeLanguage }) => {
  ...
  const headerBgClass = isScrolled || isMobileMenuOpen 
    ? 'py-4 bg-black/40 dark:bg-black/20 backdrop-blur-2xl border-b border-white/5 dark:border-white/10 shadow-sm' 
  ...
  return (
    <>
        ...
        <div className="container flex items-center justify-between">
          ...
          <nav className="hidden lg:flex items-center gap-12">
            {translations.links.map((link, i) => (
              <a 
                ...
                className="text-[11px] font-bold uppercase tracking-[0.15em] text-white/60 dark:text-white/60 hover:text-brand-blue dark:hover:text-white transition-all duration-300 relative group/link"
              >
                {link}
          ...
          <div className="flex items-center gap-3">
            ...
            <button
              ...
              className="p-2 rounded-full bg-white/5 dark:bg-white/5 border border-white/10 dark:border-white/10 hover:border-brand-blue transition-all duration-300"
              aria-label="Changer la langue"
              title="Changer la langue"
            >
              <svg className="w-4 h-4 text-white/60 dark:text-white/60" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            <button 
              ...
              className="hidden lg:block bg-white/5 dark:bg-white/5 border border-white/10 dark:border-white/10 text-white font-bold px-6 py-2.5 rounded-full text-[10px] uppercase tracking-widest hover:bg-brand-blue hover:border-brand-blue hover:text-white transition-all duration-500"
            >
              {translations.cta}
            </button>
            ...
            >
              <span className={`w-6 h-0.5 bg-transparent dark:bg-white transition-all duration-500 transform ${isMobileMenuOpen ? 'rotate-45 translate-y-2' : ''}`}></span>
              <span className={`w-6 h-0.5 bg-transparent dark:bg-white transition-all duration-500 ${isMobileMenuOpen ? 'opacity-0 scale-x-0' : ''}`}></span>
              <span className={`w-6 h-0.5 bg-transparent dark:bg-white transition-all duration-500 transform ${isMobileMenuOpen ? '-rotate-45 -translate-y-2' : ''}`}></span>
            </button>
          </div>
        </div>
      ...
      {/* ===== MOBILE MENU ===== */}
      <div className={`fixed inset-0 z-[100] bg-black/95 dark:bg-black/95 backdrop-blur-3xl flex flex-col items-center justify-center transition-all duration-700 ease-[cubic-bezier(0.16,1,0.3,1)] lg:hidden ${isMobileMenuOpen ? 'opacity-100 pointer-events-auto translate-y-0' : 'opacity-0 pointer-events-none -translate-y-full'}`}>
        <nav className="flex flex-col items-center gap-10">
          {translations.links.map((link, i) => (
            <a 
              ...
              href={`#${sectionIds[i]}`} 
              onClick={(e) => handleAnchorClick(e, sectionIds[i])}
              className={`text-3xl font-black uppercase tracking-tighter transition-all duration-700 transform text-white ${isMobileMenuOpen ? 'translate-y-0 opacity-100 blur-0' : 'translate-y-12 opacity-0 blur-md'}`}
