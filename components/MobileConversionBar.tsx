  ...
  return (
    <nav aria-label="Actions de contact rapides" className="fixed bottom-3 left-3 right-3 z-[105] lg:hidden rounded-2xl border border-white/20 bg-navy/95 p-2 shadow-2xl backdrop-blur-xl pb-[calc(0.5rem+env(safe-area-inset-bottom))]">
      <div className="grid grid-cols-3 gap-2">
        ...
        <a href="#/devis-rapide" onClick={() => trackEvent('quick_quote_click', { location: 'mobile_bar' })} className="flex min-h-12 items-center justify-center rounded-xl bg-white px-2 text-[10px] font-black uppercase tracking-wide text-navy">{t.quote}</a>
      </div>
    </nav>
  );
};
