...
const VisualCard: React.FC<{ item: ShowcaseItem; index: number; isVisible: boolean }> = ({ item, index, isVisible }) => {
  return (
    <div 
      className={`${item.gridClass} group relative overflow-hidden rounded-[30px] md:rounded-[40px] border border-white/5 dark:border-white/5 transition-all duration-[1s] ease-out shadow-2xl min-h-[300px] md:min-h-0 ${isVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-12'}`}
...
const VisualShowcase: React.FC<VisualShowcaseProps> = ({ translations }) => {
  ...
  return (
    <section ref={sectionRef} id="projets" className="py-24 md:py-40 bg-white/0 dark:bg-transparent overflow-hidden relative border-t border-white/5 dark:border-white/5 transition-colors duration-500">
