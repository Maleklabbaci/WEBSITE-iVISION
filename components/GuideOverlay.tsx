...
const GuideOverlay: React.FC<GuideOverlayProps> = ({ onClose, language }) => {
  ...
  return (
    <div className="fixed inset-0 z-[300] pointer-events-none overflow-hidden">
      ...
      >
        <div className="bg-white dark:bg-transparent p-6 rounded-[2rem] shadow-[0_20px_50px_rgba(0,0,0,0.5)] border border-white/10 dark:border-white/10 border-b-4 border-b-brand-blue animate-fade-in-up">
          ...
          <div className={`flex items-center justify-between ${isRtl ? 'flex-row-reverse' : 'flex-row'}`}>
             <button 
               onClick={onClose}
               className="text-[10px] font-black text-white/40 dark:text-white/40 hover:text-brand-blue uppercase tracking-widest transition-colors px-2 py-1"
             >
               {t.skip}
             </button>
             <button 
